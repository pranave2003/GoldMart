...
class UpiUserScreen extends StatefulWidget {
  const UpiUserScreen({
    super.key,
    required this.price,
  });
  ...
  final price;
  @override
  _UpiUserScreenState createState() => _UpiUserScreenState();
}

class _UpiUserScreenState extends State<UpiUserScreen> {
  ...
  Future<void> _initiateTransaction(UpiApp app) async {
    UpiResponse response = await _upiIndia.startTransaction(
      app: app,
      receiverUpiId: "7559099535@paytm", // Replace with the actual UPI ID
      receiverName: "vaishnav",
      transactionRefId: "TransactionID",
      transactionNote: "Payment for MD mart",
      amount: widget.price,
    );
